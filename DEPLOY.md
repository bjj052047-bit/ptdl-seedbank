# PTDL 종자실 재고 관리 - 배포 가이드

이 문서를 따라 하시면 실제 인터넷 주소(URL)로 사이트를 띄울 수 있습니다. 모두 무료 요금제 범위 안에서 진행됩니다.

---

## 1단계. Supabase 프로젝트 만들기 (데이터베이스 + 로그인)

1. https://supabase.com 접속 → GitHub 계정으로 가입/로그인
2. "New project" 클릭
   - Name: `ptdl-seedbank` (원하는 이름)
   - Database Password: 아무 강력한 비밀번호 (따로 기억해둘 필요는 없음, DB 직접 접속용)
   - Region: Northeast Asia (Seoul) 선택 권장
3. 프로젝트가 생성될 때까지 1~2분 대기
4. 왼쪽 메뉴 **SQL Editor** 클릭 → "New query"
5. 이 프로젝트의 `supabase/schema.sql` 파일 내용을 전체 복사해서 붙여넣고 **Run** 클릭
   - "Success. No rows returned" 메시지가 뜨면 성공
6. 왼쪽 메뉴 **Settings → API** 클릭 → 아래 두 값을 복사해서 메모장에 잠깐 저장:
   - `Project URL` (예: `https://xxxxx.supabase.co`)
   - `anon public` 키 (긴 문자열)

### 이메일 확인(Confirm email) 끄기 (연구실 내부용이라 간편하게)
- 왼쪽 메뉴 **Authentication → Providers → Email** 클릭
- "Confirm email" 옵션을 꺼주세요 (꺼두면 가입 즉시 로그인되어 편리합니다)

---

## 2단계. 코드 다운로드 및 환경변수 설정

1. 전달받은 `ptdl-web` 폴더를 컴퓨터에 저장
2. 폴더 안의 `.env.example` 파일을 복사해서 이름을 `.env.local` 로 바꾸기
3. `.env.local` 파일을 열어서 1단계에서 복사한 값을 채워넣기:
   ```
   NEXT_PUBLIC_SUPABASE_URL=https://xxxxx.supabase.co
   NEXT_PUBLIC_SUPABASE_ANON_KEY=eyJhbGci...(긴 문자열)
   ```

### (선택) 로컬에서 먼저 테스트해보기
컴퓨터에 Node.js가 설치되어 있다면:
```bash
cd ptdl-web
npm install
npm run dev
```
브라우저에서 `http://localhost:3000` 접속 → 정상 동작 확인

---

## 3단계. GitHub에 코드 올리기

1. https://github.com 가입/로그인 → 오른쪽 위 "+" → "New repository"
2. 이름을 `ptdl-seedbank` 로 설정 (Public 또는 Private 아무거나 무방) → "Create repository"
3. 컴�터에서:
   ```bash
   cd ptdl-web
   git init
   git add .
   git commit -m "초기 커밋"
   git branch -M main
   git remote add origin https://github.com/내계정이름/ptdl-seedbank.git
   git push -u origin main
   ```
   (GitHub Desktop 앱을 쓰신다면 명령어 없이 화면에서 클릭만으로도 가능합니다)

---

## 4단계. Vercel로 배포하기 (무료)

1. https://vercel.com 접속 → GitHub 계정으로 가입/로그인
2. "Add New..." → "Project" 클릭
3. 방금 만든 `ptdl-seedbank` 저장소를 선택 → "Import"
4. **Environment Variables** 섹션에서 아래 두 개를 추가:
   - `NEXT_PUBLIC_SUPABASE_URL` = (1단계에서 복사한 값)
   - `NEXT_PUBLIC_SUPABASE_ANON_KEY` = (1단계에서 복사한 값)
5. "Deploy" 클릭 → 1~2분 대기
6. 배포가 끝나면 `https://ptdl-seedbank-xxxx.vercel.app` 같은 실제 주소가 생성됩니다 🎉

---

## 5단계. 담당자 계정 지정하기

1. 사이트에 접속해서 본인 계정으로 "가입" (이름·이메일·비밀번호 입력)
2. Supabase 대시보드 → **Table Editor** → `profiles` 테이블 클릭
3. 방금 가입한 본인 행을 찾아서 `role` 값을 `researcher` → `staff` 로 직접 수정
4. 사이트를 새로고침하면 "담당자 전용" 탭(입출고 기록, 데이터 등록·관리)이 보이기 시작합니다

다른 팀원이 가입하면 기본적으로 `researcher`(연구원) 권한으로 시작하니, 담당자가 되어야 할 사람만 같은 방식으로 `staff`로 바꿔주시면 됩니다.

---

## 참고: 무료 요금제 한도
- **Supabase 무료**: DB 500MB, 월간 API 요청 5만 건 등 — 연구실 규모(수천 종 데이터)에는 충분합니다.
- **Vercel 무료**: 개인/소규모 프로젝트 용도로 충분한 트래픽 한도 제공.
- 나중에 사용량이 늘어나면 각 서비스의 유료 플랜으로 전환할 수 있습니다 (지금 단계에서는 필요 없음).

---

문제가 생기면 Supabase 대시보드의 **Logs** 메뉴(에러 확인)나 Vercel의 **Deployments → 로그**를 확인해보시고, 에러 메시지를 캡처해서 알려주시면 도와드리겠습니다.

---

## 기존 시스템 업데이트하기 (가입 승인 + 실험실 예약 추가)

이미 종자실 관리 시스템을 Supabase + Vercel로 운영 중이라면, 처음부터 다시 만들 필요 없이 아래 2단계만 하면 됩니다.

### 1단계. Supabase에 새 테이블/정책 추가하기
1. Supabase 대시보드 → 본인 프로젝트 → **SQL Editor** → "New query"
2. 이 프로젝트의 `supabase/schema.sql` 파일을 열어서, 아래 주석으로 시작하는 부분부터 **파일 맨 끝까지**만 복사:
   ```
   -- 마이그레이션: 연구실 통합 포털 확장
   ```
3. SQL Editor에 붙여넣고 **Run** 클릭 → "Success" 메시지 확인
   - 이 작업으로 `profiles`에 가입 승인 상태(`status`)와 이메일(`email`) 컬럼이 추가되고,
     `labs`(실험실 목록), `lab_reservations`(예약) 테이블이 새로 생깁니다.
   - **중요**: 이 SQL은 기존에 이미 가입해 있던 계정들을 자동으로 `approved`(승인됨) 상태로 만들어줍니다.
     이후 새로 가입하는 사람만 승인 대기 상태로 시작합니다.

### 2단계. 코드 반영 후 재배포
1. 전달받은 최신 `ptdl-web` 폴더 내용으로 로컬 코드를 교체 (또는 Git으로 받은 변경사항을 pull)
2. GitHub 저장소에 커밋 & 푸시:
   ```bash
   cd ptdl-web
   git add .
   git commit -m "가입 승인 + 실험실 예약 기능 추가"
   git push
   ```
3. Vercel은 GitHub 저장소와 연결되어 있으므로 push하면 자동으로 새 버전이 배포됩니다 (Vercel 대시보드에서 진행 상황 확인 가능)
4. 새 환경변수는 필요 없습니다. 기존 `NEXT_PUBLIC_SUPABASE_URL` / `NEXT_PUBLIC_SUPABASE_ANON_KEY` 그대로 사용합니다.

### 3단계. 동작 확인 체크리스트
- [ ] 기존에 쓰던 계정으로 로그인 → 예전처럼 바로 모든 탭이 보이는지 확인 (승인 화면이 뜨면 안 됨)
- [ ] 새 계정을 하나 만들어 가입 → "승인 대기중" 화면이 뜨는지 확인
- [ ] 담당자/승인자 계정으로 로그인 → 상단 **가입 승인** 탭에서 방금 만든 신규 계정이 보이고, 승인 버튼이 동작하는지 확인
- [ ] 승인 후 신규 계정으로 새로고침 → 정상적으로 메인 화면이 보이는지 확인
- [ ] **실험실 예약** 탭 → 405B / 311C 버튼 전환이 되는지 확인
- [ ] 캘린더에서 날짜 클릭 → 오른쪽에 시간 선택 칸(00-01 ~ 23-24)이 뜨는지 확인
- [ ] 연속된 시간 2~3칸을 선택해 예약 → 캘린더에 예약 건수가 표시되는지 확인
- [ ] 같은 시간대를 다른 계정(또는 같은 계정)으로 다시 예약 시도 → "이미 예약된 시간" 에러가 뜨는지 확인
- [ ] 본인 예약에는 취소 버튼이 보이고, 다른 사람 예약에는 (담당자/승인자가 아니면) 취소 버튼이 안 보이는지 확인

이 중 하나라도 예상과 다르게 동작하거나 화면에 에러 메시지(빨간 글씨)가 보이면, 어느 단계에서 무엇이 나왔는지 알려주시면 바로 확인해드리겠습니다.

---

## 기존 시스템 업데이트하기 (배드 예약 추가)

이번에 추가된 건 "배드 예약" 탭(Growth Room, Incubator 등 재배 공간을 기간 단위로 예약)입니다. 방법은 지난번과 똑같습니다.

### 1단계. Supabase에 새 테이블 추가하기
1. Supabase 대시보드 → **SQL Editor** → "New query"
2. `supabase/schema.sql` 파일에서 **`-- 마이그레이션: 배드(재배 공간) 예약 시스템`** 줄부터 파일 맨 끝까지 복사
3. SQL Editor에 붙여넣고 **Run** 클릭 → "Success" 메시지 확인
   - `beds`(배드 42개 목록, Growth Room/Incubator/Growth Chamber 등 8개 시설) 테이블과 `bed_reservations`(기간 예약) 테이블이 생성됩니다.

### 2단계. 코드 반영 후 재배포
```bash
cd ptdl-web
git add .
git commit -m "배드 예약 기능 추가"
git push
```
Vercel이 자동으로 재배포합니다. 새 환경변수는 필요 없습니다.

### 3단계. 동작 확인 체크리스트
- [ ] 상단 메뉴에 **배드 예약** 탭이 보이는지 확인
- [ ] 배드 선택 드롭다운에 시설별로 그룹핑되어(Growth Room 415, Incubator 415 등) 배드들이 보이는지 확인
- [ ] 배드 하나 선택 → 시작일/종료일 선택 → "예약하기"로 정상 예약되는지 확인
- [ ] 예약한 배드의 작은 달력에 해당 날짜들이 보라색으로 표시되는지 확인 (칸에 마우스 올리면 예약자 이름 툴팁으로 확인)
- [ ] 같은 배드, 겹치는 기간으로 다시 예약 시도 → "OO 기간과 겹칩니다. 공유하도록 협의하셨나요?" 확인창이 뜨는지, "취소"를 누르면 예약이 안 되고 "확인"을 누르면 겹친 채로 예약되는지 확인
- [ ] 페이지 아래 **나의 배드 예약 내역**에 방금 만든 예약이 보이고 취소가 되는지 확인
- [ ] **실험실 예약** 탭 맨 아래에도 **나의 예약 내역** 섹션이 새로 생겼는지 확인

문제가 있으면 몇 번째 항목에서 무엇이 다르게 나왔는지 알려주세요.

---

## 기존 시스템 업데이트하기 (배드 예약 - 승인 절차, 작물/비고 분리, 수정·종료 기능)

이번 업데이트 내용: 배드 예약에 담당자/승인자 승인 절차 추가, 재배 작물/용도/비고 분리 입력, 실험실·배드 "나의 예약 내역"에서 직접 수정 가능, 배드 예약 조기 종료 기능, 미니 달력 확대(클릭으로 날짜 선택 가능).

### 1단계. Supabase에 변경사항 반영하기
1. Supabase 대시보드 → **SQL Editor** → "New query"
2. `supabase/schema.sql` 파일에서 **`-- 마이그레이션: 배드 예약 - 작물/비고 분리, 승인 절차, 예약 수정 허용`** 줄부터 파일 맨 끝까지 복사
3. SQL Editor에 붙여넣고 **Run** 클릭 → "Success" 메시지 확인
   - `bed_reservations`에 `crop`(작물) / `notes`(비고) / `status`(승인 상태) 컬럼이 추가되고, 기존에 이미 등록되어 있던 예약은 자동으로 `approved`(승인됨) 처리됩니다.
   - `lab_reservations`, `bed_reservations` 둘 다 수정(UPDATE)이 가능하도록 권한이 추가됩니다 (이전에는 취소만 가능했음).

### 2단계. 코드 반영 후 재배포
```bash
cd ~/Desktop/ptdl-web
git add .
git commit -m "배드 예약 승인 절차, 작물/비고 분리, 예약 수정·종료 기능 추가"
git push
```
Vercel이 자동으로 재배포합니다. 새 환경변수는 필요 없습니다.

### 3단계. 동작 확인 체크리스트
- [ ] **배드 예약** 페이지 상단 달력이 이전보다 커지고, 날짜를 클릭하면 시작일/종료일 입력칸에도 반영되는지 확인
- [ ] 시작일/종료일 입력칸에 직접 타이핑도 여전히 되는지 확인
- [ ] 재배 작물 / 용도 / 비고 3칸이 각각 따로 보이는지 확인
- [ ] 배드 예약 신청 → 바로 확정되지 않고 "승인 대기중" 상태로 뜨는지 확인
- [ ] 담당자/승인자 계정으로 로그인 → **배드 예약** 페이지에 "승인 대기 중인 배드 예약" 목록이 보이고, 승인/거절이 동작하는지 확인
- [ ] 승인 후 "나의 배드 예약 내역"과 "이 배드 예약 목록"에서 상태가 "승인됨"으로 바뀌는지 확인
- [ ] "나의 배드 예약 내역"에서 "수정" 버튼으로 기간/작물/용도/비고를 바꾸고 저장하면, 상태가 다시 "대기중"으로 바뀌는지 확인
- [ ] "실험실 예약" 페이지의 "나의 예약 내역"에서도 "수정" 버튼으로 날짜/시간/용도를 바꿀 수 있는지 확인
- [ ] 예약 하나를 골라 "종료" 버튼 클릭 → 오늘 날짜(또는 원하는 날짜)를 입력 → 종료일이 앞당겨지고, 그 이후 날짜가 배드 캘린더에서 다시 예약 가능한(흰색) 상태로 보이는지 확인

문제가 있으면 몇 번째 항목에서 무엇이 다르게 나왔는지 알려주세요.

---

## 기존 시스템 업데이트하기 (실험실 예약 - 실험 목적 + 사용 기기 선택)

이번 업데이트 내용: 실험실 예약 시 실험 목적(6종) 선택 + 사용할 기기 선택 기능 추가, 같은 기기가 겹치는 시간에는 예약이 아예 안 되도록 차단(빨간 경고문 표시). 그리고 같은 실험실·같은 시간대를 여러 팀이 함께 예약할 수 있도록 기존 제한을 풀었습니다 (실제로 여러 팀이 한 공간을 같이 쓰는 경우가 많았기 때문).

### 1단계. Supabase에 변경사항 반영하기
1. Supabase 대시보드 → **SQL Editor** → "New query"
2. `supabase/schema.sql` 파일에서 **`-- 마이그레이션: 실험실 예약 - 실험 목적 + 사용 기기(장비) 예약`** 줄부터 파일 맨 끝까지 복사
3. SQL Editor에 붙여넣고 **Run** 클릭 → "Success" 메시지 확인
   - 기존 "같은 실험실·같은 시간대는 한 팀만" 제약이 제거되어 여러 팀이 동시에 예약할 수 있게 됩니다.
   - `lab_devices`(311C 6종, 405B 8종 기기 목록)와 `lab_reservation_devices`(예약-기기 연결, 기기+시간 겹침 방지) 테이블이 새로 생깁니다.

### 2단계. 코드 반영 후 재배포
```bash
rm -f ~/Desktop/ptdl-web/.git/index.lock
cd ~/Desktop/ptdl-web
git add .
git commit -m "실험실 예약에 실험 목적 + 사용 기기 선택 기능 추가"
git push
```
Vercel이 자동으로 재배포합니다. 새 환경변수는 필요 없습니다.

### 3단계. 동작 확인 체크리스트
- [ ] **실험실 예약** 페이지에서 같은 날짜·같은 시간대에 서로 다른 두 계정이 각자 예약해도 (기기를 다르게 선택하면) 둘 다 성공하는지 확인
- [ ] 시간 선택 후 "실험 목적" 6개 항목이 라디오 버튼으로 보이는지, "기타" 선택 시 직접 입력 칸이 나오는지 확인
- [ ] 실험 목적을 고르면 그 아래 "사용할 기기" 버튼 목록이 나타나는지 확인 (311C 선택 시 6개, 405B 선택 시 8개)
- [ ] 이미 누군가 특정 시간에 예약해 둔 기기를, 겹치는 시간대에 다시 선택 → 빨간 글씨로 "OO 기기가 몇시~몇시에 이미 예약되어 있습니다 (이름)" 경고가 뜨고 예약 버튼이 비활성화되는지 확인
- [ ] 겹치지 않는 기기를 선택하면 정상적으로 예약되는지 확인
- [ ] "이 날짜 예약 현황"과 "나의 예약 내역" 표에 실험 목적/사용 기기 열이 보이는지 확인
- [ ] "나의 예약 내역"에서 "수정" 버튼으로 실험 목적과 기기까지 바꿀 수 있는지, 바꿀 때도 기기 겹침 검사가 동작하는지 확인

문제가 있으면 몇 번째 항목에서 무엇이 다르게 나왔는지 알려주세요.
