...
export default function ManagePage() {
  ...
  async function handleExportExcel() {
    ...
    try {
      ...
      const txSheet = (txData || []).map((t) => ({
        일시: t.created_at, 종자코드: t.seeds?.code || '', 구분: t.type,
        '수량(g)': t.qty, '처리후재고(g)': t.qty_after, 담당자: t.by_name, 비고: t.note,
      }));
    ...
    <div className="wrap">
      ...
      <div className="card">
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', flexWrap: 'wrap', gap: 10 }}>
          <h4 className="serif" style={{ margin: 0 }}>전체 데이터 ({allSeeds.length}건)</h4>
          <button className="btn btn-ghost" onClick={handleExportExcel} disabled={exportBusy}>
            {exportBusy ? '내보내는 중...' : '전체 데이터 백업 (엑셀)'}
          </button>
        </div>
        <p style={{ fontSize: 11.5, color: '#847d68', marginTop: 6 }}>
          종자 목록·입출고기록·요청내역을 각각 시트로 담은 엑셀 파일이 즉시 다운로드됩니다. 정기적으로 받아두시면 만약의 상황에도 데이터를 복구할 수 있습니다.
        </p>
